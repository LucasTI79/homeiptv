...
export function VodPage() {
  ...
  const clearAllProgress = usePlaybackStore((s) => s.clearAllProgress);
  ...
  const [filterType, setFilterType] = useState<'all' | 'movie' | 'series' | 'favorites' | 'watched'>('all');
    ...
    if (item.type === 'movie') {
      if (item.url) {
        ...
        if (isPaired && role === 'client') {
          ...
          toast.success(`Transmitindo "${item.name}" na TV!`, { icon: '📺' });
          return;
        }
  ...
  const handlePlayEpisode = (options: PlayEpisodeOptions) => {
    ...
    if (isPaired && role === 'client') {
      ...
      toast.success(`Transmitindo "${options.title}" na TV!`, { icon: '📺' });
      return;
    }
  ...
  const handleResumeContinueWatching = (cw: VodProgressItem) => {
    ...
    if (isPaired && role === 'client') {
      ...
      toast.success(`Transmitindo "${cw.title}" na TV!`, { icon: '📺' });
      return;
    }
        ...
        <section className="mb-8 bg-gray-800/40 border border-gray-700/50 rounded-2xl p-4 md:p-5 backdrop-blur-xs">
          <div className="flex items-center justify-between mb-3.5 flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={async () => {
                if (window.confirm('Deseja realmente limpar todos os itens em andamento do Continuar Assistindo?')) {
                  await clearAllProgress();
                  toast.success('Continuar Assistindo limpo!');
                }
              }}
              className="px-2.5 py-1 text-xs text-gray-400 hover:text-rose-400 hover:bg-rose-500/10 border border-gray-700/60 hover:border-rose-500/30 rounded-lg transition flex items-center gap-1.5 shadow-sm"
              title="Limpar todos os itens em andamento"
            >
                  ...
                  <div className="relative aspect-video bg-gray-950 overflow-hidden">
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        toast.success(`"${cw.title}" removido de Continuar Assistindo!`);
                      }}
                      className="absolute top-1.5 right-1.5 p-1 rounded-md bg-black/60 hover:bg-red-600 text-gray-300 hover:text-white transition opacity-70 group-hover:opacity-100 z-10"
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6 items-stretch md:items-center" id="vod-filters">
        <div className="flex bg-gray-800 rounded p-1 shrink-0">
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded text-sm font-medium transition flex items-center gap-1.5 ${filterType === 'watched' ? 'bg-emerald-600 text-white' : 'text-gray-400 hover:text-white'}`}
          >
            <FiCheck className="w-3.5 h-3.5 text-emerald-300" />
            Assistidos ({watchedItemsCount})
          </button>
        </div>
